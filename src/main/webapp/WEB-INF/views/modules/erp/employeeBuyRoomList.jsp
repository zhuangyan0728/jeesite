...
<body>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/erp/employee/">人才申请购房信息列表</a></li>
		<%--<shiro:hasPermission name="erp:employee:edit"><li><a href="${ctx}/erp/employee/form">企业人才信息添加</a></li></shiro:hasPermission>--%>
	</ul>
	<form:form id="searchForm" modelAttribute="employee" action="${ctx}/erp/employee/buylist" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="employee">
			<tr>
				...
				<td><a href="${ctx}/erp/employee/buyform?id=${employee.id}">
					${employee.name}
				</a></td>
				...
				<td>
				 ...
				 <td>
					<c:if test="${employee.ifBuyApplyAudit =='0'}">
						未审核
					</c:if>
					<c:if test="${employee.ifBuyApplyAudit =='1'}">
						审核通过
					</c:if>
				...
				<td> <a href="javascript:void(0);" onclick ="printRoomApply('${employee.id}')">购房申请打印</a>&nbsp;&nbsp;&nbsp;&nbsp;</td>
				<shiro:hasPermission name="erp:employee:edit">
					<td>
						<a href="${ctx}/erp/employee/cancelBuyApply?id=${employee.id}" onclick="return confirmx('确认要发起申请？', this.href)">撤销购房申请</a>
					</td>
				</shiro:hasPermission>
