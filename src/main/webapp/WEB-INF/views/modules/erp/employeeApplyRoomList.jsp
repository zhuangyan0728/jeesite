			...
			$.jBox.open(html2, "拒绝", 500, 300, { showType: "show" , buttons: { "拒绝": true, "取消": false },
				submit:function (v, h, f) {
					if (v == true) {
						...
						$.ajax({
							url: "${ctx}/erp/employee/ajustApply?id="+userid+"&messageIn="+encodeURIComponent(encodeURIComponent($("#messageIn").val())),
							type: 'POST',
							data: formData,
							async: false,
							cache: false,
			...
			<tr>
				...
				<td>
					...
					<c:if test="${employee.ifApplyAudit =='2'}">
						拒绝申请<c:if test="${not empty employee.reason1}">
						<label>（原因：${employee.reason1}）</label>
					</c:if>
					</c:if>
				...
				<shiro:lacksPermission name="erp:employee:edit">
					<td>
						...
						<%--<a href="${ctx}/erp/employee/ajustApply?id=${employee.id}" onclick="return confirmx('确认要拒绝该申请吗？', this.href)">--%>
							<%--拒绝申请</a>--%>
						<a id="jsRecordBackEdit" href="javascript:void(0)" onclick="test('${employee.id}')" value="${employee.id}">拒绝申请
